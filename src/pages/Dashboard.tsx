...
import { Users, Briefcase, TrendingUp, Award, Target, BookOpen, MessageSquare, BarChart3, ArrowRight, Sparkles, Building2, Calendar } from "lucide-react";
...
export default function Dashboard() {
  ...
  const fetchUserAndStats = async () => {
    try {
      ...
      const {
        data: roleData
      } = await supabase.from('user_roles').select('role').eq('user_id', authUser.id).single();
      if (profile) {
        setUser({
          ...profile,
      ...
      // Get dashboard stats
      const [studentsCount, jobsCount, assessmentsCount] = await Promise.all([supabase.from('students').select('*', {
        count: 'exact',
        head: true
      }), supabase.from('job_postings').select('*', {
        count: 'exact',
        head: true
      }).eq('is_active', true), supabase.from('assessments').select('*', {
        count: 'exact',
        head: true
      })]);
      ...
      const {
        data: placedStudents
      } = await supabase.from('students').select('placement_readiness_score').eq('placement_status', 'placed');
      const {
        data: allStudentsReadiness
      } = await supabase.from('students').select('placement_readiness_score').not('placement_readiness_score', 'is', null);
      const placementRate = studentsCount.count ? Math.round((placedStudents?.length || 0) / studentsCount.count * 100) : 0;
      const averageReadiness = allStudentsReadiness?.length ? Math.round(allStudentsReadiness.reduce((sum, student) => sum + (student.placement_readiness_score || 0), 0) / allStudentsReadiness.length) : 0;
      setStats({
      ...
      const {
        data: topPlaced
      } = await supabase.from('students').select('name, placed_company, placed_role, package_lpa, department').eq('placement_status', 'placed').not('package_lpa', 'is', null).order('package_lpa', {
        ascending: false
      }).limit(5);
      ...
      const {
        data: recentPlaced
      } = await supabase.from('students').select('name, placed_company, placed_role, package_lpa, department').eq('placement_status', 'placed').not('package_lpa', 'is', null).order('updated_at', {
        ascending: false
      }).limit(5);
      ...
      const {
        data: skills
      } = await supabase.from('skill_analysis').select('skill_name, current_demand, predicted_demand, trend, job_count').order('current_demand', {
        ascending: false
      }).limit(8);
      ...
      const {
        data: jobsByIndustry
      } = await supabase.from('job_postings').select('industry').eq('is_active', true);
  ...
  return <div className="space-y-8 animate-fade-in relative">
        ...
        <div className="relative p-8 lg:p-12">
          <div className="max-w-2xl">
            <div className="flex items-center gap-2 mb-4">
              ...
              <Badge variant="secondary" className="text-xs bg-lime-500">
                AI-Powered Insights
              </Badge>
            </div>
            ...
            <div className="flex flex-wrap gap-3">
              {quickActions.map((action, index) => <Button key={index} onClick={action.action} variant={action.variant === "primary" ? "default" : "outline"} className={`${action.variant === "primary" ? "gradient-primary glow-hover" : "glass-card hover:bg-secondary/50"} transition-smooth`}>
                  {action.icon}
      ...
      {/* Stats Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {roleBasedStats.map((stat, index) => <StatsCard key={index} title={stat.title} value={stat.value} description={stat.description} icon={stat.icon} variant={stat.variant} className="animate-fade-in" />)}
      </div>

      {/* Quick Actions Cards */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {quickActions.map((action, index) => <Card key={index} className="glass-card card-hover cursor-pointer transition-smooth" onClick={action.action}>
            <CardHeader className="flex flex-row items-center space-y-0 pb-2">
              <div className="flex items-center space-x-3">
                <div className={`p-2 rounded-lg ${action.variant === "primary" ? "gradient-primary text-white" : "bg-secondary/50"}`}>
                  {action.icon}
                </div>
                <div>
      ...
      <Card className="glass-card">
        ...
        <CardContent>
          <div className="space-y-4">
            {topPlacements.length > 0 ? topPlacements.map((student, index) => <div key={index} className="flex items-center justify-between p-3 rounded-lg bg-secondary/20 hover:bg-secondary/30 transition-smooth">
                ...
                </div>) : <div className="text-center text-muted-foreground py-8">No placement data available</div>}
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="glass-card">
        ...
        <CardContent>
          <div className="space-y-3">
            {recentPlacements.length > 0 ? recentPlacements.map((student, index) => <div key={index} className="flex items-center justify-between p-3 rounded-lg border border-border hover:bg-secondary/20 transition-smooth">
                  <div className="flex items-center gap-3">
                ...
                </div>) : <div className="text-center text-muted-foreground py-8">No recent placements</div>}
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="glass-card">
          <CardHeader>
            <div className="flex items-center space-x-3">
              ...
              <div>
                <CardTitle className="text-emerald-50">Job Trends by Industry</CardTitle>
                <CardDescription>Active job postings distribution</CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            {jobTrendsData.length > 0 ? <ResponsiveContainer width="100%" height={300}>
                <BarChart data={jobTrendsData}>
                  <CartesianGrid strokeDasharray="3 3" opacity={0.1} />
                  <XAxis dataKey="industry" tick={{
                  fontSize: 12
              ...
              </ResponsiveContainer> : <div className="text-center text-muted-foreground py-12">No job data available</div>}
          </CardContent>
        </Card>
        ...
        <Card className="glass-card">
          ...
          <CardContent>
            {skillTrends.length > 0 ? <ResponsiveContainer width="100%" height={300}>
                <LineChart data={skillTrends}>
                  <CartesianGrid strokeDasharray="3 3" opacity={0.1} />
                  <XAxis dataKey="skill_name" tick={{
                  fontSize: 12
                }} angle={-45} textAnchor="end" height={80} />
                  <YAxis />
                  <Tooltip contentStyle={{
              ...
              </ResponsiveContainer> : <div className="text-center text-muted-foreground py-12">No skill data available</div>}
          </CardContent>
        </Card>
      </div>
      </div>
